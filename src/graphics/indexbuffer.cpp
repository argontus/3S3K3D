...
:   numElements_(numElements),
    elementSize_(elementSize(format)),
    id_(0),
    previousId_(0),
    format_(format),
    usage_(usage)
{
    ...
    if (id_ != previousId)
    {
        // restore previous buffer binding so that constructing an index buffer
...
int IndexBuffer::elementSize(const Format::Enum format)
{
    switch (format)
    {
...
GLbitfield IndexBuffer::convert(const Access::Enum access)
{
    switch (access)
    {
...
GLenum IndexBuffer::convert(const Usage::Enum usage)
{
    switch (usage)
    {
        case Usage::Dynamic:
