...
:   meshManager_(meshManager),
    vertexBufferManager_(vertexBufferManager),
    meshPrefix_(),
    effect_(0)
{
    GRAPHICS_RUNTIME_ASSERT(meshManager != 0);
    GRAPHICS_RUNTIME_ASSERT(vertexBufferManager != 0);
}

Node* ModelReader::read(const std::string& path, const Effect& effect)
{
    GRAPHICS_RUNTIME_ASSERT(meshManager_ != 0);
    GRAPHICS_RUNTIME_ASSERT(vertexBufferManager_ != 0);
...
void ModelReader::readMesh(const Lib3dsMesh* const p, const std::string& prefix)
{
    ...
    for (int i = 0; i < numFaces; ++i)
    {
        ...
        mesh->vertex(i * 3 + 0).position = Vector3(p->vertices[i0][0], p->vertices[i0][1], p->vertices[i0][2]);
        mesh->vertex(i * 3 + 1).position = Vector3(p->vertices[i1][0], p->vertices[i1][1], p->vertices[i1][2]);
        mesh->vertex(i * 3 + 2).position = Vector3(p->vertices[i2][0], p->vertices[i2][1], p->vertices[i2][2]);
...
//        mesh->vertices()[i * 3 + 2] = Vector3(p->vertices[i2][0], p->vertices[i2][1], p->vertices[i2][2]);
        ...
        if (p->texcos != 0)
        {
            mesh->vertex(i * 3 + 0).texCoord = Vector2(p->texcos[i0][0], p->texcos[i0][1]);
            mesh->vertex(i * 3 + 1).texCoord = Vector2(p->texcos[i1][0], p->texcos[i1][1]);
            mesh->vertex(i * 3 + 2).texCoord = Vector2(p->texcos[i2][0], p->texcos[i2][1]);
...
//            mesh->texCoords()[i * 3 + 2] = Vector2(p->texcos[i2][0], p->texcos[i2][1]);
        ...
        {
            mesh->vertex(i * 3 + 0).texCoord = Vector2::zero();
            mesh->vertex(i * 3 + 1).texCoord = Vector2::zero();
            mesh->vertex(i * 3 + 2).texCoord = Vector2::zero();
...
//            mesh->texCoords()[i * 3 + 2] = Vector2(0.0f, 0.0f);
    ...
    vertexBufferManager_->loadResource(prefix + meshName, vertexBuffer);
}
...
Node* ModelReader::readMeshNode(const Lib3dsNode* const p)
{
    ...
    Mesh* const mesh = meshManager_->getResource(meshPrefix_ + meshName);
    ...
    VertexBuffer* const vertexBuffer = vertexBufferManager_->getResource(meshPrefix_ + meshName);
    GRAPHICS_RUNTIME_ASSERT(vertexBuffer != 0);
